...
WITH source_data AS (
    ...
    SELECT
        ...
        timezone(
            'UTC', to_timestamp("ValidFrom", 'DD/MM/YYYY hh24:mi')::TIMESTAMP
        ) AS "CustomersPurchaseTime"
...
group_label AS (
    ...
    SELECT
        ...
        row_number()
            OVER (ORDER BY "CustomersID", "CustomersPurchaseTime")
        ::INTEGER AS "CustomersGroupID",
...
add_grouping AS (
    ...
    SELECT
        ...
        j."CustomersOnes",
        sum(j."CustomersOnes")
            OVER (PARTITION BY m."CustomersID" ORDER BY m."CustomersPurchaseTime")
        ::INTEGER AS "CustomersGroupID"
    FROM add_group_discriminator AS m
    LEFT JOIN group_label AS j
        ON
            m."CustomersID" = j."CustomersID"
            AND m."CustomersPurchaseTime" = j."CustomersPurchaseTime"
...
add_grouping_index AS (
    ...
    SELECT
        *,
        ...
        ::INTEGER AS "CustomersGroupIndex"
    FROM add_grouping
